...
export default function ConfusingToggle() {
  ...
  const handleToggleChange = () => {
    ...
    if (clickCount % 3 === 2) {
      ...
      setMessage(`Click ${3 - (clickCount % 3)} more time${(clickCount % 3) === 1 ? "" : "s"} to toggle`)
    }
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center space-x-2">
        <Switch id="confusing-toggle" checked={visuallyChecked} onCheckedChange={handleToggleChange} />
      ...
      <p className="text-xs text-muted-foreground italic">
        This toggle requires 3 clicks to actually change state. The visual feedback is misleading.
      </p>
    </div>
  )
}
